...
void main() {
    ...
    float n = noise(vec3(gl_FragCoord.xy*8./R.y, .1*iTime));
    float v = sin(6.28*10.*n);
    float t = iTime;
    
    v = smoothstep(1.,0., .5*abs(v)/fwidth(v));
    
	vec4 O = mix( exp(-33./R.y )* texture( iChannel0, (gl_FragCoord.xy+vec2(1,sin(t)))/R), .5+.5*sin(12.*n+vec4(0,2.1,-2.1,0)), v );
