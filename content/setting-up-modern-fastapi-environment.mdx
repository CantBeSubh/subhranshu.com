...
Linting that only runs in CI catches problems an hour too late. A `pre-commit` git hook catches them before the commit even exists. [Husky](https://typicode.github.io/husky/) is normally a JS tool, but it's a perfectly good place to manage hooks for a *whole* repo, even a Python backend living in a subdirectory - especially once a frontend shows up alongside it.
